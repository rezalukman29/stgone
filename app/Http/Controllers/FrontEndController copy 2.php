...
class FrontEndController extends JoshController
{
    ...
    public function postLogin(FrontendLoginRequest $request)
    {
        ...
        try {
            ...
            if ($user=  Sentinel::authenticate($request->only(['nik', 'password']), $request->get('remember-me', 0))) {
                ...
                return redirect('login')->with('error', 'NIK or password is incorrect.');
            }
        } catch (UserNotFoundException $e) {
            $this->messageBag->add('nik', trans('auth/message.account_not_found'));
        } catch (NotActivatedException $e) {
            $this->messageBag->add('nik', trans('auth/message.account_not_activated'));
        } catch (UserSuspendedException $e) {
            $this->messageBag->add('nik', trans('auth/message.account_suspended'));
        } catch (UserBannedException $e) {
            $this->messageBag->add('nik', trans('auth/message.account_banned'));
        } catch (ThrottlingException $e) {
            $delay = $e->getDelay();
            $this->messageBag->add('nik', trans('auth/message.account_suspended', compact('delay')));
        }
    ...
    public function postRegister(UserRequest $request)
    {
        ...
        try {
            // Register the user
            $user = Sentinel::register($request->only(['first_name', 'last_name', 'email', 'password', 'gender','nik']), $activate);
    ...
    public function postContact(Request $request)
    {
        $data = [
            ...
            'contact-nik' => $request->get('contact-nik'),
