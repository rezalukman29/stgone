...
class FrontDataAbsent extends Controller
{
    ...
    public function index()
    {
        ...
        // $period=Period::pluck('period', 'period');
        // $period['all']='Select All';


        // $max_count =Datatable::all()->count();   // We can pass max count for slider
        // $max_id =Datatable::pluck('id')->max();
        // $min_id =Datatable::pluck('id')->min();
        // return view('daily_absensi', compact('professions', 'period','max_count', 'max_id', 'min_id'));
        $period = Period::pluck('period', 'period');
        return view('daily_absensii', compact('period'));
    }
